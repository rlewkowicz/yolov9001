...
output "ssh_key_pem_path" {
  ...
  value       = length(var.ssh_key_pair) == 0 ? "${local.ssh_key_pair_path}/${module.ssh_key_pair[0].key_name}.pem" : null
}
...
output "security_group_id" {
  value       = local.security_group_enabled ? module.security_group.id : null
  description = "EC2 instances Security Group ID"
}

output "security_group_arn" {
  value       = local.security_group_enabled ? module.security_group.arn : null
  description = "EC2 instances Security Group ARN"
}

output "security_group_name" {
  value       = local.security_group_enabled ? module.security_group.name : null
