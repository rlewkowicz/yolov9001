...
resource "aws_instance" "default" {
  ...
  # This now correctly references the conditional module
  key_name                    = length(var.ssh_key_pair) > 0 ? var.ssh_key_pair : module.ssh_key_pair[0].key_name
...
module "ssh_key_pair" {
  # This module will now only be created if `var.ssh_key_pair` is not provided
  count                 = length(var.ssh_key_pair) == 0 ? 1 : 0
