    ...
    def fuse(self):
        ...
        if isinstance(detect_head, RN_DualDDetect) and hasattr(detect_head, 'cv2'):
            ...
            while q:
                ...
                if isinstance(from_indices, int): from_indices = [from_indices]
...
class DetectionModel(BaseModel):
    ...
    def _forward_augment(self, x):
        ...
        for si, fi in zip(s, f):
            ...
            yi = output[0][1] if isinstance(output, tuple) and isinstance(output[0], list) else output
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if m in {
                ...
                nn.ConvTranspose2d, DWConvTranspose2d, SPPCSPC, ADown, ELAN1, RepNCSPELAN4, SPPELAN,
                QARepVGGBlockV2, SimCSPSPPF, GeMSPPF, GatedSPPF, ASPP, QARepNCSPELAN}:
            c1, c2 = ch[f], args[0]
            if c2 != no:
            ...
            if m in {BottleneckCSP, SPPCSPC}:
                args.insert(2, n)
                n = 1
        elif m in {RepBlock, RepBlockAdd}:
            c1, c2 = ch[f], args[0]
            if c2 != no:
                c2 = make_divisible(c2 * gw, 8)
