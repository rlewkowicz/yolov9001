...
import torch
import yaml
from torch.optim import lr_scheduler, AdamW, SGD, Optimizer
...
class EMAStepOptimizer(Optimizer):
    """
    A wrapper for an optimizer that applies an EMA update after each step.
    ...
    """
    def __init__(self, optimizer, ema, model):
        self.optimizer = optimizer
        self.ema = ema
        self.model = model
        self.param_groups = self.optimizer.param_groups
        self.state = self.optimizer.state  # Schedulers also check for the state attribute
...
class Trainer:
    ...
    def __init__(self, opt, device):
        ...
        real_optimizer = self._setup_optimizer()
        self.ema = ModelEMA(self.model)
        self.optimizer = EMAStepOptimizer(real_optimizer, self.ema, self.model)
        ...
        if getattr(opt, "tb", False):
            try:
                ...
                LOGGER.info(
                    f"TensorBoard logging → {self.save_dir / 'tb'} (run: tensorboard --logdir {self.save_dir.parent})"
                )
    ...
    def _setup_optimizer(self):
        ...
        lr_scale_sppf = dec.get("sppf",
                                {}).get("lr_scale", (lr_scale_backbone + lr_scale_head) / 2.0)

        param_groups = {
            "backbone_weights": {
                "params": [], "lr_scale": lr_scale_backbone, "weight_decay":
                    optimizer_settings.get("weight_decay", 0.0)
            },
            "backbone_others": {"params": [], "lr_scale": lr_scale_backbone, "weight_decay": 0.0},
            "sppf_weights": {
                "params": [], "lr_scale": lr_scale_sppf, "weight_decay":
                    optimizer_settings.get("weight_decay", 0.0)
            },
            "sppf_others": {"params": [], "lr_scale": lr_scale_sppf, "weight_decay": 0.0},
            "head_weights": {
                "params": [], "lr_scale": lr_scale_head, "weight_decay":
                    optimizer_settings.get("weight_decay", 0.0)
            },
        ...
        for name, p in self.model.named_parameters():
            ...
            prefix = "sppf" if module_index == sppf_index else (
                "backbone" if module_index < backbone_len else "head"
            )
        ...
        optimizer_param_groups = [{
            ...
            "initial_lr": base_lr * g["lr_scale"], "weight_decay": g["weight_decay"], "name": name
        ...
        if self.opt.optimizer == 'AdamW':
            ...
            return SGD(
                optimizer_param_groups, momentum=optimizer_settings['momentum'], nesterov=True
            )
        elif self.opt.optimizer == 'LION':
            return Lion(
                optimizer_param_groups,
                betas=(optimizer_settings['b1'], optimizer_settings['b2']),
                **optimizer_settings.get("lion_extras", {})
            )
        else:
    ...
    def _setup_scheduler(self):
        ...
        lf_original = lf_map.get(
            lr_scheduler_type, lambda *args:
            (lambda x: (1 - x / self.opt.epochs) * (1.0 - lrf) + lrf)
        )(1, lrf, self.opt.epochs)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.opt.epochs):
            ...
            LOGGER.info(
                f"Epoch {epoch+1:>3}/{self.opt.epochs} | loss(box/cls/dfl): {mloss[0]:.4f}/{mloss[1]:.4f}/{mloss[2]:.4f} | mP {mp:.4f} mR {mr:.4f} mAP50 {map50:.4f} mAP {map_:.4f}"
            )
    ...
    def _train_epoch(self, epoch, nw, last_opt_step):
        ...
        for i, (imgs, targets, _, _) in enumerate(pbar):
            ...
            if ni <= nw:
                ...
                self.accumulate = int(
                    max(1,
                        np.interp(ni, xi, [1, self._nbs / max(1, self.opt.batch_size)]).round())
                )
                for pg in self.optimizer.param_groups:
                    start_lr = optim_set.get("warmup_bias_lr",
                                             0.0) if "others" in pg.get("name", "") else 0.0
                    pg["lr"] = float(
                        np.interp(ni, xi, [start_lr, pg["initial_lr"] * self._lf_epoch(epoch)])
                    )
                    if 'momentum' in pg:
                        pg['momentum'] = float(
                            np.interp(
                                ni, xi,
                                [optim_set.get('warmup_momentum', 0.0), optim_set['momentum']]
                            )
                        )
                    if 'betas' in pg:
                        pg['betas'] = (
                            float(
                                np.interp(
                                    ni, xi,
                                    [optim_set.get('warmup_momentum', 0.0), optim_set['b1']]
                                )
                            ), pg['betas'][1]
                        )
    ...
    def export_graph(self):
        ...
        try:
            ...
            model_to_export = self.ema.ema._orig_mod if hasattr(
                self.ema.ema, '_orig_mod'
            ) else self.ema.ema
