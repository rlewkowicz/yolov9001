...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        if min_items:
            include = (
                np.array([len(x) >= min_items for x in self.labels]).nonzero()[0].astype(int)
            )
    ...
    def __getitem__(self, index):
        ...
        if nl:
            labels[:, 1:5] = xyxy2xywhn(
                labels[:, 1:5], w=img.shape[1], h=img.shape[0], clip=True, eps=0.001
            )
        if self.augment:
