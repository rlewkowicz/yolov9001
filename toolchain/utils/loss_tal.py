...
class RN_ComputeLoss(nn.Module):
    ...
    def _calculate_loss_for_branch(self, feats, targets, imgsz):
        ...
        targets_preprocessed = self.preprocess(targets, batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        ...
        if self.use_dfl:
            ...
            pixel_pred_dist = pixel_pred_dist.view(b, a, 4, c//4).softmax(3).matmul(self.proj.type(pixel_pred_dist.dtype))
        ...
        target_labels, target_bboxes, target_scores, fg_mask = self.assigner(
            pred_scores.detach().sigmoid(), pred_bboxes_for_assigner.type(gt_bboxes.dtype),
        ...
        if fg_mask.sum():
            ...
            loss[0], loss[2] = self.bbox_loss(pred_distri, pred_bboxes_decoded, anchor_points,
                                                target_bboxes, target_scores, target_scores_sum, fg_mask)
        return loss

    def __call__(self, p, targets, img=None, epoch=0):
        feats = p[1] if isinstance(p, tuple) else p
        imgsz = torch.tensor(feats[0][0].shape[2:], device=self.device, dtype=torch.float32) * self.stride[0]
