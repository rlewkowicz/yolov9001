...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    ...
    if fg_mask.max() > 1:
        ...
        is_max_overlaps = F.one_hot(max_overlaps_idx, n_max_boxes).permute(0, 2,
                                                                           1).to(overlaps.dtype)
...
class TaskAlignedAssigner(nn.Module):
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        if topk_mask is None:
            topk_mask = (topk_vals.max(-1, keepdim=True).values
                         > self.eps).expand(-1, -1, self.topk)
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask):
        ...
        target_scores = torch.where(
            fg_scores_mask > 0, target_scores, torch.zeros_like(target_scores)
        )
