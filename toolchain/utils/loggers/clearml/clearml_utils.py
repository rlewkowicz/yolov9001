...
class ClearmlLogger:
    ...
    def log_image_with_boxes(self, image_path, boxes, class_names, image, conf_threshold=0.25):
        ...
        """
        if (
            len(self.current_epoch_logged_images) < self.max_imgs_to_log_per_epoch and
            self.current_epoch >= 0
        ):
            if (
                self.current_epoch % self.bbox_interval == 0 and
                image_path not in self.current_epoch_logged_images
            ):
