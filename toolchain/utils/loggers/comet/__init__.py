...
class CometLogger:
    ...
    def __init__(self, opt, hyp, run_id=None, job_type="Training", **experiment_kwargs) -> None:
        ...
        self.upload_dataset = (
            self.opt.upload_dataset if self.opt.upload_dataset else COMET_UPLOAD_DATASET
        )
        ...
        self.log_parameters({
            "val_iou_threshold": self.iou_thres, "val_conf_threshold": self.conf_thres
        })
    ...
    def update_data_paths(self, data_dict):
        ...
        for split in ["train", "val", "test"]:
            if data_dict.get(split):
                ...
                data_dict[split] = (
                    f"{path}/{split_path}"
                    if isinstance(split, str) else [f"{path}/{x}" for x in split_path]
                )
    ...
    def on_val_batch_end(self, batch_i, images, targets, paths, shapes, outputs):
        if not (
            self.comet_log_predictions and ((batch_i + 1) % self.comet_log_prediction_interval == 0)
        ):
            return
    ...
    def on_model_save(self, last, epoch, final_epoch, best_fitness, fi):
        if ((epoch + 1) % self.opt.save_period == 0 and
            not final_epoch) and self.opt.save_period != -1:
