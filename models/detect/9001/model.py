...
class v9001(DetectModelBase):
    ...
    def _compute_layer_args(self, m, f_m, args, ch, depth, width, max_channels, threshold):
        ...
        if m is WFPB:
            enabled = bool(self.config_obj.get('wfpb_enabled', True))
            k = int(args[1]) if len(args) > 1 else 3
            rr = float(args[2]) if len(args) > 2 else 0.5
            if not enabled:
                return [c1, k, rr], c1, nn.Identity()
            return [c1, k, rr], c1, None
        if m is DWConv:
            c2 = min(int(args[0] * width), max_channels)
            return [c1, c2, *args[1:]], c2, None
        if m is CISBA:
            reduction = int(args[1]) if len(args) > 1 else 16
            spatial_k = int(args[2]) if len(args) > 2 else 7
            return [c1, reduction, spatial_k], c1, None
